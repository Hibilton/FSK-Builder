...
def load_data(path: str) -> pd.DataFrame:
    ...
    for c in ["ShaftSize","SternSize","Priority","Clamp1_Qty","Clamp2_Qty",
    ...
    for c in ["ShaftUnits","SternUnits","FSA_Template","Hose_Code",
              "Clamp1_Code","Clamp2_Code","Stretch_Type","Comments"]:
...
def main():
    ...
    shaft=st.sidebar.selectbox("Shaft size", shaft_vals,
    ...
    stern=st.sidebar.selectbox("Stern tube OD", stern_vals,
    ...
    view["Priority Meaning"] = view["Priority"].map(PRIORITY_MAP).fillna("Custom rule")

    table_cols=["Option","FSA_Template","Hose_Code","Clamp_Backend","Clamp_Stern","Total_Clamps_Per_Kit","Priority","Priority Meaning"]

    st.dataframe(view[table_cols], use_container_width=True, hide_index=True)

    with st.expander("Advanced", expanded=False):
        adv_cols=["Option","Is_US_Recommended","Stretch_Type",
                  "Stretch_Backend_in","Stretch_Stern_in","Comments"]
