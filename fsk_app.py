...
def load_csv(path: str) -> pd.DataFrame:
    ...
    for c in ["ShaftUnits","SternUnits","FSA_Template","Hose_Code","Clamp1_Code","Clamp2_Code","Hat_SKU","Stretch_Type","Comments"]:
...
def option_rows(row, stocked_sets=None):
    ...
    hat_line = f'{hat} × {int(row["Hat_Qty"]) if pd.notna(row["Hat_Qty"]) else 1}' if hat else ""
    ...
    if stocked_sets:
        ...
        if hat:
            rows.insert(9, ("Hat status", badge("HAT", hat)))
...
def main():
    ...
    stocked_sets = {
        ...
        "HAT": set(df_stocked["Hat_SKU"].dropna().astype(str).str.strip().tolist()),
